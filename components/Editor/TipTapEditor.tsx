...
export default function TipTapEditor({ content, onChange }: TipTapEditorProps) {
  ...
  const editor = useEditor({
    ...
    onUpdate: ({ editor }) => {
      // CRITICAL: Before getting HTML, ensure all image styles and attributes are synced to DOM
      // ReactNodeViewRenderer serializes from actual DOM, so attributes must be on elements
      if (typeof window !== 'undefined') {
        const editorElement = editor.view.dom as HTMLElement
        const images = editorElement.querySelectorAll('img')
        
        // Build a map of image nodes from editor state
        const imageNodeMap = new Map<string, any>()
        editor.state.doc.descendants((node) => {
          if (node.type.name === 'image' && node.attrs.src) {
            imageNodeMap.set(node.attrs.src, node.attrs)
          }
        })
        ...
        images.forEach((img) => {
          ...
          // CRITICAL: Sync data-spoiler attribute from editor state to DOM
          const nodeAttrs = imageNodeMap.get(src)
          if (nodeAttrs) {
            if (nodeAttrs['data-spoiler'] === 'true' || nodeAttrs['data-spoiler'] === true) {
              imgElement.setAttribute('data-spoiler', 'true')
            } else {
              imgElement.removeAttribute('data-spoiler')
            }
          }
